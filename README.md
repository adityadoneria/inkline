...
Built by Aditya Doneria with SvelteKit + TypeScript + D3.
...
- Data: CSV/XLSX upload, paste-from-Excel, bundled sample datasets, editable data grid with
  per-column type detection/override
...
- Style: title/subtitle/source/footer, palette, legend toggle, dark mode, responsive vs. fixed size
- Export: PNG (1x/2x/transparent), JPEG, SVG, PDF, self-contained interactive HTML with
  ...
  export — all client-side
- Local library: chart projects persist to IndexedDB via Dexie

**Not yet built:** the rest of the chart catalog (pie/donut, scatter, table, maps, histogram,
box plot, etc.), a full annotation/label-collision system, themes-as-files, i18n/RTL, keyboard
shortcuts & command palette, PDF batch export, and export presets.
...
(GitHub Pages, Netlify, Vercel, S3) — no accounts, no server, no database.
